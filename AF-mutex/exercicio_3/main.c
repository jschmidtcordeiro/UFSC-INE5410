...
// Recebe o ponteiro para a variavel na qual o valor sera incrementado
// Desse modo nao eh necessario utilizar uma variavel global
// visto que cada thread tera sua variavel acumuladora
void compute(int arg, int *count) {
    if (arg < 2) {
        *count += arg;
    } else {
        compute(arg - 1, count);
