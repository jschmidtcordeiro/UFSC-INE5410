...
int veiculos_turno;
int chegaram = 0;
sem_t sem_sai_continente, sem_sai_ilha, sem_mutex;
...
void ponte_inicializar() {
	...
	sem_init(&sem_sai_continente, 0, 4);
	sem_init(&sem_sai_ilha, 0, 0);
...
void ponte_entrar(veiculo_t *v) {
	if (v->cabeceira == 0) {
		sem_wait(&sem_sai_continente);
	} else if (v->cabeceira == 1) {
		sem_wait(&sem_sai_ilha);
	}
}
...
void ponte_sair(veiculo_t *v) {
	...
	if (chegaram == 4) {
		...
		for (int i = 0; i < veiculos_turno; i++) {
			if (v->cabeceira == 0) {
				sem_post(&sem_sai_continente);
			} else if (v->cabeceira == 1) {
				sem_post(&sem_sai_ilha);
			}
		}
	}
	sem_post(&sem_mutex);
...
void ponte_finalizar() {
	...
	sem_destroy(&sem_sai_continente);
	sem_destroy(&sem_sai_ilha);
